<?php
	...
	if(isset($_GET['submitbtn']))
	{
		...
		$sql="select * from admin_master where adminid='".$adminid."'";
		include 'db.php';
		$i=mysql_query($sql);
		if($arr=mysql_fetch_array($i))
		{
			if($arr["adminid"]==$adminid && $arr["password"]==$oldpassword)
			{
				$sql1="update admin_master set password='".$newpassword."' where adminid='".$adminid."' and password='".$oldpassword."'";
				include 'db.php';
				$j=mysql_query($sql1);
				if($j)
				{
				  	...
				  	<h2>Change Password(Admin)</h2>
					    <form method="get" >
					    	<div>
						    	...
						    	<span><input type="password" class="form-control" id="oldpassword" name="oldpassword"></span>
						    </div>
						    <div>
